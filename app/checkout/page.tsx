...
//       const response = fetch(`http://localhost:3000/customer_orders`, {
//         method: "POST",
//         headers: {
...
//     const response = await fetch(`http://localhost:3000/order-product`, {
//       method: "POST", // or 'PUT'
//       headers: {
//         "Content-Type": "application/json",
//       },
...
import { SectionTitle } from "@/components";
import { useProductStore } from "../_zustand/store"; // Adjust the import based on your store structure
import Image from "next/image";
import { useEffect, useState } from "react";
import toast from "react-hot-toast";
import { useRouter } from "next/navigation";
...
} from "@/lib/utils"; // Ensure these utility functions are correctly implemented
...
const CheckoutPage = () => {
  ...
  const makePurchase = async () => {
    ...
    if (!Object.values(checkoutForm).every((field) => field.length > 0)) {
    ...
    try {                           
      const response = await fetch(`http://localhost:3000/api/customer_orders`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="bg-white">
      ...
      <main className="relative mx-auto grid max-w-screen-2xl grid-cols-1 gap-x-16 lg:grid-cols-2 lg:px-8 xl:gap-x-48">
            ...
            <ul role="list" className="divide-y divide-gray-200 text-sm font-medium text-gray-900">
              {products.map((product) => (
                <li key={product?.id} className="flex items-start space-x-4 py-6">
                  <Image src={product?.image ? `/${product?.image}` : "/product_placeholder.jpg"} alt={product?.title} width={80} height={80} className="h-20 w-20 flex-none rounded-md object-cover object-center" />
                  <div className="flex-auto space-y-1">
                    <h3>{product?.title}</h3>
                    <p className="text-gray-500">x{product?.amount}</p>
                  </div>
        ...
        <form className="px-4 pt-16 sm:px-6 lg:col-start-1 lg:row-start-1 lg:px-0" onSubmit={(e) => { e.preventDefault(); makePurchase(); }}>
          <div className="mx-auto max-w-lg lg:max-w-none">
            <section aria-labelledby="contact-info-heading">
              ...
              <div className="mt-6">
                ...
                <div className="mt-1">
                  <input type="text" id="name-input" name="name-input" value={checkoutForm.name} onChange={(e) => setCheckoutForm({ ...checkoutForm, name: e.target.value })} className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" />
                </div>
              </div>
              {/* Additional input fields for lastname, phone, email, etc. */}
              <div className="mt-6">
...
<div className="mt-6">
  ...
  <div className="mt-1">
    ...
    />
            ...
            {/* Payment and Shipping sections go here */}
            <section aria-labelledby="payment-heading" className="mt-10">
    ...
    <div className="col-span-2 sm:col-span-3">
      ...
      <div className="mt-1">
        <input
          ...
          name="expiration-date"
          value={checkoutForm.expirationDate}
          onChange={(e) => setCheckoutForm({ ...checkoutForm, expirationDate: e.target.value })}
          className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
        />
      </div>
    </div>
    ...
    <div className="sm:col-span-3">
      ...
      <div className="mt-1">
        ...
        ></textarea>
            ...
            <div className="mt-10 border-t border-gray-200 pt-6 ml-0">
              <button type="button" onClick={makePurchase} className="w-full rounded-md border border-transparent bg-blue-500 px-20 py-2 text-lg font-medium text-white shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:ring-offset-gray-50 sm:order-last">Pay Now</button>
            </div>
          </div>
        </form>
      </main>
