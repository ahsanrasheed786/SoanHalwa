...
//     const [fatched , setFatched]=useState([])
...
//             `http://localhost:3000/api/customer_order/${params.id}`
//           );
...
//         try {
//           const response = await fetch(
//             `http://localhost:3000/api/customer_order_product/${params.id}`
//           );
...
const AdminSingleOrder = () => {
  ...
  useEffect(() => {
    const fetchOrderProducts = async () => {
      if (params?.id) {
        try {
          const response = await fetch(`/api/customer_order_product/${params.id}`);
          if (!response.ok) throw new Error("Failed to fetch order products");
          const data: OrderProduct[] = await response.json();
          setOrderProducts(data);
          // console.log(orderProducts?.customerOrderId)
        } catch (error) {
          toast.error("Error fetching order products");
        }
      }
    ...
    const fetchUser = async () => {
      if (params?.id) {
        try {
          const response = await fetch(`/api/customer_orders/${params.id}`);
          if (!response.ok) throw new Error("Failed to fetch order products");
          const data: Order[] = await response.json();
          setOrder(data);
         } catch (error) {
  ...
  const updateOrder = async () => {
    if (
      ...
      if (!isValidNameOrLastname(order?.name)) {
        toast.error("You entered an invalid name format");
        return;
      }

      if (!isValidNameOrLastname(order?.lastname)) {
        toast.error("You entered an invalid lastname format");
        return;
      }

      if (!isValidEmailAddressFormat(order?.email)) {
        toast.error("You entered an invalid email format");
        return;
      }

      fetch(`/api/customer_orders/${params.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        .catch((error) => toast.error("There was an error while updating the order"));
    } else {
      toast.error("Please fill all fields");
    }
  };
  ...
  const deleteOrder = async () => {
    ...
    fetch(`/api/customer_order_product/${OrderProduct.customerOrderId}`, requestOptions).then(() => {
      fetch(`/api/api/customer_orders/${order.id}`, requestOptions).then(() => {
        toast.success("Order deleted successfully");
        router.push("/admin/orders");
      });
    });
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div className="mt-5">
          <label className="w-full">
            <div>
              <span className="text-xl font-bold">Order ID:</span>
              <span className="text-base"> {orderProducts[0]?.customerOrderId}</span>
               {/* {console.log(order)} */}
             </div>
          </label>
        </div>

        {/* Rest of the form and layout */}
        <div className="flex gap-x-2 max-sm:flex-col">
          <div>
            <label className="form-control w-full max-w-xs">
              <div className="label">
